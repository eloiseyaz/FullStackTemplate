...
sealed abstract class APIError(val httpResponseStatus: Int, val reason: String) extends Exception
...
object APIError {
  ...
  final case class NotFoundResponse(upstreamStatus: Int, upstreamMessage: String) extends APIError(
    Status.NOT_FOUND,
    s"Bad response from upstream; Status: $upstreamStatus, Reason: $upstreamMessage"
  )
